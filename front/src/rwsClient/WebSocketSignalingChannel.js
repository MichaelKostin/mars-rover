...
export default function WebSocketSignalingChannel(connectButton,disconnectButton,remoteVideo, wsPath ) {
...
WebSocketSignalingChannel.prototype.connectWebSocket = function () {
  ...
  if( this.isPrivateIPaddress_( window.location.host ) || window.location.hostname === 'localhost')
    websocket_url = `ws://192.168.100.4:8809${websocket_url_path}`;
  else
    websocket_url = "wss://" + window.location.host + websocket_url_path;
...
WebSocketSignalingChannel.prototype.onWebSocketOpen_ = function (event) {
  console.log("Websocket connnected: " + this.websocket_.url);
...
WebSocketSignalingChannel.prototype.WebSocketSendMessage = function (message) {
  ...
  console.log("failed to send websocket message :" + message);
...
WebSocketSignalingChannel.prototype.doSignalingConnect = function () {
  ...
  if( window.WebSocket ){
    ...
    this.peerConnectionClient_ = new PeerConnectionClient(this.remoteVideo_, this.doSignalingSend.bind(this) );
    this.connectWebSocket();
  }else{
    console.log("doSignalingConnect: WebSocket is not suppported in this platform!");
