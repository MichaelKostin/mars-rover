...
function PeerConnectionClient(remoteVideo, doSendPeerMessage) {
  ...
  setTimeout(() => {
    ...
    window.hello = this.peerConnection_;
    this.peerConnection_.getStats(null)
      .then(res => {
        console.log(res);
        console.log(res.forEach(stat => stat.packetsSent))

      })
    console.log(this.peerConnection_.getConfiguration())
    console.log(this.peerConnection_.getSenders())
  }, 3000)
};
